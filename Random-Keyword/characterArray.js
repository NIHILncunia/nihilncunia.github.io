...
function btnrace2() {
  var race2dice = classRace2.race2[Math.floor(Math.random() * classRace2.race2.length)];
  var btnresult = document.getElementById("btnresult");
  var btntext = '오늘의 종족은 <span class="color">' + race2dice + "!</span> (" + classRace2.race2.length + "개)";
  btnresult.innerHTML = btntext;
} // 종족

function btnclass2() {
  var class2dice = classRace2.class2[Math.floor(Math.random() * classRace2.class2.length)];
  var btnresult = document.getElementById("btnresult");
  var btntext = '오늘의 직업은 <span class="color">' + class2dice + "!</span> (" + classRace2.class2.length + "개)";
  btnresult.innerHTML = btntext;
} // 직업
...
function btncharacterImage() {
  ...
  var btnresult = document.getElementById("btnresult");
  var btntext = '오늘의 캐릭터 유형은 <span class="color">' + characterImagedice + "!</span> (" + btncharacterImage.length + "개)";
  btnresult.innerHTML = btntext;
} // 캐릭터 유형
...
function btnsuperpower() {
  ...
  var btnsuperpower = [
    ...
    '타인에게 빙의할 수 있는', '음식을 섭취하지 않아도 생존할 수 있는', '큰 소리를 내서 음파를 방출하는', '죽으면 특정 과거로 이동하는', '투명해질 수 있는',
  ];
...
function btnbackstory() {
  ...
  var 누구 = classRace2.race2.concat(classRace2.class2);
