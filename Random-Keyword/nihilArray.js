...
function btnrace() {
  var racedice = 종족클래스.종족[Math.floor(Math.random() * 종족클래스.종족.length)];
  var btnresult = document.getElementById("btnresult");
  var btntext = '오늘의 종족은 <span class="color">' + racedice + "!</span> (" + 종족클래스.종족.length + "개)";
  btnresult.innerHTML = btntext;
} // 니힐라 종족

function btnclass() {
  var classdice = 종족클래스.클래스[Math.floor(Math.random() * 종족클래스.클래스.length)];
  var btnresult = document.getElementById("btnresult");
  var btntext = '오늘의 직업은 <span class="color">' + classdice + "!</span> (" + 종족클래스.클래스.length + "개)";
  btnresult.innerHTML = btntext;
} // 니힐라 직업
...
function btnwhatIllust() {
  var btnwhatIllust = [
    '팬아트를 그려라', '리퀘스트를 그려라', '세계관 캐릭터를 그려라', '오늘은 쉬도록 하라', '랜덤 디자인을 해라',
	  ...
	  '세계관 캐릭터 리메이크를 해라', '쉬어도 좋지만 구상은 해라',
  ];
...
function btnNihilbackstory() {
  ...
  var 누구 = 종족클래스.종족.concat(종족클래스.클래스);
