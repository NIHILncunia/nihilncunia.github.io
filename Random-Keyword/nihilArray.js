...
function btnconsonantSet() {
	...
	var 끝음절2 = [
		...
		'ur', 'ir', 'eion', 'eium', 'eil',
		'aos', 'iel', 'asas', 'asis', 'asus',
		'ases', 'asos', 'af', 'if', 'ef',
		'uf', 'of', 'ian', 'ein', 'ain',
		'el', 'onel', 'anel', 'enel', 'inel',

		'ia', 'ea', 'im', 'eim', 'ulis',
		'ulos', 'ulas', 'ules', 'cles', 'clos',
		'clis', 'clas',
	];

	var dice1 = 첫음절1[Math.floor(Math.random() * 첫음절1.length)];
	var dice2 = 첫음절2[Math.floor(Math.random() * 첫음절2.length)];

	var dice3 = 중음절1[Math.floor(Math.random() * 중음절1.length)];
	var dice4 = 중음절2[Math.floor(Math.random() * 중음절2.length)];

	var dice5 = 중음절3[Math.floor(Math.random() * 중음절3.length)];
	var dice6 = 중음절4[Math.floor(Math.random() * 중음절4.length)];

	var dice7 = 끝음절1[Math.floor(Math.random() * 끝음절1.length)];
	var dice8 = 끝음절2[Math.floor(Math.random() * 끝음절2.length)];

	var consonant1 = dice1 + dice2;
	var consonant2 = dice3 + dice4;
	var consonant3 = dice5 + dice6;
	var consonant4 = dice7 + dice8; // 끝음절

	var consonantDice = Math.floor(Math.random() * 10 + 1);
	var consonantSet;

	switch (consonantDice) {
		case 1:
			consonantSet = consonant4;
			break;
		case 2:
			consonantSet = `${consonant1} + ${consonant4}`;
			break;
		case 3:
			consonantSet = `${consonant2} + ${consonant4}`;
			break;
		case 8:
			consonantSet = `${consonant3} + ${consonant4}`;
			break;
		case 4:
			consonantSet = `${consonant2} + ${consonant3} + ${consonant4}`;
			break;
		case 5:
			consonantSet = `${consonant1} + ${consonant3} + ${consonant4}`;
			break;
		case 6:
			consonantSet = `${consonant3} + ${consonant2} + ${consonant4}`;
			break;
		case 9:
			consonantSet = `${consonant3} + ${consonant1} + ${consonant4}`;
			break;
		case 7:
			consonantSet = `${consonant1} + ${consonant2} + ${consonant3} + ${consonant4}`;
			break;
		case 10:
			consonantSet = `${consonant3} + ${consonant1} + ${consonant2} + ${consonant4}`;
			break;
	}

	var btnresult = document.getElementById("btnresult");
	var btntext = `
		오늘의 이름은 <span class="color">${consonantSet}!</span><br />
		<span style="font-size: 14pt;">버튼이 던져준 단어는 매우 심각할 정도로 불안정하기에 이름 영감을 받는다는 느낌으로 만족하자.</span>
	`;
	btnresult.innerHTML = btntext;
} // 자모음 조합
